/// @description scr_Depot_TOW_Tab(target_unit, tow_ammo, tow_max, tow_rate);
/// @param target_unit
/// @param  tow_ammo
/// @param  tow_max
/// @param  tow_rate
function scr_Depot_TOW_Tab(argument0, argument1, argument2, argument3) {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if towamo_d_amount <= (dpt_amo_supply-towS_supply_rate) {
	                ...
	                if ((towamo_d_amount+towS_count)+towS_supply_rate) <= tow_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((towamo_d_amount+towS_supply_rate)/towS_supply_rate)*ap_cost {
	                            //Add the rounds
	        ...
	        if towamo_d_amount < 0 { towamo_d_amount = 0; }
	        if towamo_d_amount > dpt_amo_supply { towamo_d_amount = dpt_amo_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (towamo_d_amount/towS_supply_rate)*ap_cost;
	    ...
	    if global.my_turn == true {
	        if f0 { 
				...
				if ds_list_size(tabs) > 1 {
	                ...
	                //TRANSFER TOW AMMO
	                //Remove ammo cost
	                if towamo_d_amount != 0 {
	                    var t=0;
	                    while (towamo_d_amount >= towS_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                            ...
	                            if (u.can_shoot == true) && (u.ammunition_ammo >= towS_supply_rate) {
	                                if (u.action_points >= ap_cost) {
	                                    ...
	                                    if towamo_d_amount >= 0 { 
	                                        ...
	                                        u.shoot_amount    += 1; 
	                                        u.ammunition_ammo -= towS_supply_rate;
