	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if global.rprbld_l_amount <= (global.logi_bld_supply-repairS_supply_rate) { 
	                //Make sure the unit rpg ammo does not go over max
	                if ((global.rprbld_l_amount+repair_count)+repairS_supply_rate) <= repair_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((global.rprbld_l_amount+repairS_supply_rate)/repairS_supply_rate)*ap_cost {
	                            //Add the rounds
	                            global.rprbld_l_amount +=  repairS_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if global.rprbld_l_amount >= repairS_supply_rate { 
	                global.rprbld_l_amount -= repairS_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if global.rprbld_l_amount < 0 { global.rprbld_l_amount = 0; }
	        if global.rprbld_l_amount > global.logi_bld_supply { global.rprbld_l_amount = global.logi_bld_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (global.rprbld_l_amount/repairS_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough supply available
	                if global.rprmg_l_amount <= (global.logi_amo_supply-repairmg_supply_rate) {
	            ...
	            if global.rprmg_l_amount > global.logi_amo_supply { global.rprmg_l_amount = global.logi_amo_supply; }
	            //Add to temp AP cost
	    ...
	    if global.my_turn == true {
	        if f0 { 
				...
				if global.selected_logib != 0 {
	                ...
	                //Remove cost
	                if global.rprbld_l_amount != 0 {
	                    var t=0;
	                    while (global.rprbld_l_amount >= repairS_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_logia_list); i+=1) {
