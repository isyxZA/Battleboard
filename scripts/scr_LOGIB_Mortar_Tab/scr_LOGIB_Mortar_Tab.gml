/// @description scr_LOGIB_Mortar_Tab(target_unit, mortar_ammo, mortar_max, mortar_rate);
/// @param target_unit
/// @param mortar_ammo
/// @param mortar_max
/// @param mortar_rate
function scr_LOGIB_Mortar_Tab(argument0, argument1, argument2, argument3) {
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if mtramo_l_amount <= (logib_amo_supply-mortarS_supply_rate) {
	                //Make sure the unit ammo does not go over max
	                if ((mtramo_l_amount+mortarS_count)+mortarS_supply_rate) <= mortar_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((mtramo_l_amount+mortarS_supply_rate)/mortarS_supply_rate)*ap_cost {
	                            //Add the rounds
	                            mtramo_l_amount += mortarS_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if mtramo_l_amount >= mortarS_supply_rate { 
	                mtramo_l_amount -= mortarS_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if mtramo_l_amount < 0 { mtramo_l_amount = 0; }
	        if mtramo_l_amount > logib_amo_supply { mtramo_l_amount = logib_amo_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (mtramo_l_amount/mortarS_supply_rate)*ap_cost;
	    ...
	    if global.my_turn == true {
	        if f0 { 
				...
				if ds_list_size(tabs) > 1 {
	                ...
	                //Remove ammo cost
	                if mtramo_l_amount != 0 {
	                    var t=0;
	                    while (mtramo_l_amount >= mortarS_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_logib_list); i+=1) {
