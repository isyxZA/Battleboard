/// @description scr_Depot_Repair_Tab(target_unit, repair_ammo, mg_ammo, repair_max, mg_max, repair_rate, mg_rate);
/// @param target_unit
/// @param  repair_ammo
/// @param  mg_ammo
/// @param  repair_max
...
/// @param  repair_rate
/// @param  mg_rate
function scr_Depot_Repair_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6) {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() || keyboard_check_pressed(global.INCREASE) { 
	            ...
	            if rprbld_d_amount <= (dpt_bld_supply-repairS_supply_rate) { 
	                ...
	                if ((rprbld_d_amount+repair_count)+repairS_supply_rate) <= repair_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((rprbld_d_amount+repairS_supply_rate)/repairS_supply_rate)*ap_cost {
	                            //Add the rounds
	        ...
	        if rprbld_d_amount < 0 { rprbld_d_amount = 0; }
	        if rprbld_d_amount > dpt_bld_supply { rprbld_d_amount = dpt_bld_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (rprbld_d_amount/repairS_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() || keyboard_check_pressed(global.INCREASE) { 
	                ...
	                if rprmg_d_amount <= (dpt_amo_supply-repairmg_supply_rate) {
	                    ...
	                    if ((rprmg_d_amount+repairmg_count)+repairmg_supply_rate) <= mg_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((rprmg_d_amount+repairmg_supply_rate)/repairmg_supply_rate)*ap_cost {
	                                //Add the rounds
	            ...
	            //Clamp the value between zero and the max available rounds
	            if rprmg_d_amount < 0 { rprmg_d_amount = 0; }
	            if rprmg_d_amount > dpt_amo_supply { rprmg_d_amount = dpt_amo_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (rprmg_d_amount/repairmg_supply_rate)*ap_cost;
	            //Set the menu position
	    ...
	    if global.my_turn == true {
	        if f0 { 
				...
				if ds_list_size(tabs) > 1 {
	                ...
	                //Remove cost
	                if rprbld_d_amount != 0 {
	                    var t=0;
	                    while (rprbld_d_amount >= repairS_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                            ...
	                            if (u.can_shoot == true) && (u.parts_ammo >= repairS_supply_rate) {
	                                if (u.action_points >= ap_cost) {
	                                    ...
	                                    if rprbld_d_amount >= 0 { 
	                                        ...
	                                        u.shoot_amount   += 1; 
	                                        u.parts_ammo     -= repairS_supply_rate;
	                ...
	                queue_reset = true;
	                    ...
	                    if rprmg_d_amount != 0 {
	                        var t=0;
	                        while (rprmg_d_amount >= repairmg_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_depot_list, i);
	                                if (u.can_shoot == true) && (u.ammunition_ammo >= repairmg_supply_rate) {
	                                    if (u.action_points >= ap_cost) {
	                                        ...
	                                        if rprmg_d_amount >= 0 { 
	                                            ...
	                                            u.shoot_amount    += 1; 
	                                            u.ammunition_ammo -= repairmg_supply_rate;
	                                            global.turn_AP    -= ap_cost;
	                                            u.action_points   -= ap_cost;
	                                            u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                        }
