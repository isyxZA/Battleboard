/// @description scr_LOGIA_LAVA_Tab(target_unit, he_ammo, ap_ammo, mg_ammo, at_ammo, he_max, ap_max, mg_max, at_max, he_rate, ap_rate, mg_rate, at_rate);
/// @param target_unit
/// @param he_ammo
/// @param ap_ammo
/// @param mg_ammo
...
/// @param mg_rate
/// @param at_rate
function scr_LOGIA_LAVA_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6, argument7, argument8, argument9, argument10, argument11, argument12) {
	...
	*/
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if lavhe_l_amount > logia_amo_supply { lavhe_l_amount = logia_amo_supply; }
	        //Add to temp AP cost
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if lavap_l_amount > logia_amo_supply { lavap_l_amount = logia_amo_supply; }
	            //Add to temp AP cost
	            ...
	            obj_ACTIONMENU.display_menuinfo = true;
		            ...
		            if lavmg_l_amount > logia_amo_supply { lavmg_l_amount = logia_amo_supply; }
		            //Add to temp AP cost
		            ...
		            obj_ACTIONMENU.display_menuinfo = true;
			            ...
			            if mouse_wheel_up() || keyboard_check_pressed(global.INCREASE) { 
			                ...
			                if lavat_l_amount <= (logia_amo_supply-lavat_supply_rate) { 
			                    //Make sure the unit rpg ammo does not go over max
			                    if ((lavat_l_amount+at_count)+lavat_supply_rate) <= at_max {
			                        //Check if there is enough turn AP for the move
			                        var m_ap = (global.turn_AP-global.temp_AP);
			                        if m_ap >= ap_cost {
			                            //Check total unit AP available
			                            ...
			                            if ap >= ((lavat_l_amount+lavat_supply_rate)/lavat_supply_rate)*ap_cost {
			                                //Add the rounds
			            ...
			            //Clamp the value between zero and the max available rounds
			            if lavat_l_amount < 0 { lavat_l_amount = 0; }
			            if lavat_l_amount > logia_amo_supply { lavat_l_amount = logia_amo_supply; }
			            //Add to temp AP cost
			            global.temp_AP = (lavat_l_amount/lavat_supply_rate)*ap_cost;
			            //Set the menu position
	                ...
	                queue_reset = true;
	                    ...
	                    queue_reset = true;
		                    ...
		                    queue_reset = true;
			                    ...
			                    if lavat_l_amount != 0 {
			                        var t=0;
			                        while (lavat_l_amount >= lavat_supply_rate) {
			                            t+=1;
			                            var i;
			                            for (i=0; i<ds_list_size(global.selected_logia_list); i+=1) {
			                                var u = ds_list_find_value(global.selected_logia_list, i);
			                                if (u.can_shoot == true) && (u.ammo_supply >= lavat_supply_rate) {
			                                    if (u.action_points >= ap_cost) {
			                                        lavat_l_amount -= lavat_supply_rate;
			                                        if lavat_l_amount >= 0 { 
			                                            if u.shoot_amount == 0 { 
			                                            ...
			                                            u.shoot_amount   += 1; 
			                                            u.ammo_supply    -= lavat_supply_rate;
			                                            global.turn_AP   -= ap_cost;
			                                            u.action_points  -= ap_cost;
			                                            u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
			                                        }
