function scr_Repair_Fire_Menu() {
	...
	if global.selected_tank != 0 || global.selected_engineer != 0 || global.selected_btr != 0 
	    || global.selected_logi != 0 || global.selected_depot != 0 || global.selected_infantry != 0  
	...
	draw_text_colour_shadow(global.menu_x+(global.h_spacing*0.5), (global.menu_y - global.v_spacing*0.5), "REPAIR", c_gray, c_gray, c_gray, c_gray, 1, 0, c_black, 1);
	    ...
	    if (m < array_length_1d(r_fire_menu)-2) {
	        ...
	        draw_text_colour_shadow(global.menu_x+(global.h_spacing*0.5), (global.menu_y+(global.v_spacing*0.5)) + (m * global.v_spacing), r_fire_menu[m] + " " + "x" + " " + string(shoot_amount), c1, c1, c1, c1, 1, 300, c_black, 0.2);
	            ...
	            var c2;
	                ...
	                //For RE-TARGET
	                draw_text_colour_shadow(global.menu_x+(global.h_spacing*0.5), (global.menu_y+(global.v_spacing*0.5)) + (m * global.v_spacing), r_fire_menu[m], c2, c2, c2, c2, 1, 300, c_black, 0.2);
	        }
	            else {
	                var c3;
	                if m == global.fire_option { c3 = c_red; } else { c3 = c_gray; }
	                //For CANCEL
	                draw_text_colour_shadow(global.menu_x+(global.h_spacing*0.5), (global.menu_y+(global.v_spacing*0.5)) + (m * global.v_spacing), r_fire_menu[m], c3, c3, c3, c3, 1, 300, c_black, 0.2);
	            }
