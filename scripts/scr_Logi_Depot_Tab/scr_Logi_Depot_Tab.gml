/// @description scr_Logi_Depot_Tab(target_unit, parts_ammo, ammunition_ammo, parts_max, ammunition_max, parts_rate, ammunition_rate);
/// @param target_unit
/// @param  parts_ammo
...
function scr_Logi_Depot_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6) {
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if global.depotParts_l_amount <= (global.building_supply-depotparts_supply_rate) { 
	                //Make sure the unit rpg ammo does not go over max
	                if ((global.depotParts_l_amount+depotparts_count)+depotparts_supply_rate) <= parts_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((global.depotParts_l_amount+depotparts_supply_rate)/depotparts_supply_rate)*ap_cost {
	                            //Add the rounds
	                            global.depotParts_l_amount +=  depotparts_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if global.depotParts_l_amount >= depotparts_supply_rate { 
	                global.depotParts_l_amount -= depotparts_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if global.depotParts_l_amount < 0 { global.depotParts_l_amount = 0; }
	        if global.depotParts_l_amount > global.building_supply { global.depotParts_l_amount = global.building_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (global.depotParts_l_amount/depotparts_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough supply available
	                if global.depotAmmo_l_amount <= (global.ammo_supply-depotammo_supply_rate) {
	                    //Make sure the unit ammo does not go over max
	                    if ((global.depotAmmo_l_amount+depotammo_count)+depotammo_supply_rate) <= ammunition_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((global.depotAmmo_l_amount+depotammo_supply_rate)/depotammo_supply_rate)*ap_cost {
	                                //Add the rounds
	                                global.depotAmmo_l_amount +=  depotammo_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            if mouse_wheel_down() { 
	                if global.depotAmmo_l_amount >= depotammo_supply_rate { 
	                    global.depotAmmo_l_amount -= depotammo_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if global.depotAmmo_l_amount < 0 { global.depotAmmo_l_amount = 0; }
	            if global.depotAmmo_l_amount > global.ammo_supply { global.depotAmmo_l_amount = global.ammo_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (global.depotAmmo_l_amount/depotammo_supply_rate)*ap_cost;
	            //Set the menu position
	    ...
	    if global.my_turn == true {
	        if f0 { 
	            ...
	            if global.selected_depot != 0 {
	                ...
	                //Remove cost
	                if global.depotParts_l_amount != 0 {
	                    var t=0;
	                    while (global.depotParts_l_amount >= depotparts_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                            var u = ds_list_find_value(global.selected_logi_list, i);
	                            if (u.can_shoot == true) && (u.building_supply >= depotparts_supply_rate) {
	                                if (u.action_points >= ap_cost) {
	                                    ...
	                                    if global.depotParts_l_amount >= 0 { 
	                                        ...
	                                        u.shoot_amount    += 1; 
	                                        u.building_supply -= depotparts_supply_rate;
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove ammo cost
	                    if global.depotAmmo_l_amount != 0 {
	                        var t=0;
	                        while (global.depotAmmo_l_amount >= depotammo_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_logi_list, i);
	                                if (u.can_shoot == true) && (u.ammo_supply >= depotammo_supply_rate) {
	                                    if (u.action_points >= ap_cost) {
	                                        global.depotAmmo_l_amount -= depotammo_supply_rate;
	                                        if global.depotAmmo_l_amount >= 0 { 
	                                            if u.shoot_amount == 0 { 
	                                            ...
	                                            u.shoot_amount   += 1; 
	                                            u.ammo_supply    -= depotammo_supply_rate;
	                                            global.turn_AP   -= ap_cost;
	                                            u.action_points  -= ap_cost;
	                                            u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                        }
