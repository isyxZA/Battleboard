/// @description scr_SupplyShip_Depot_Tab(target_unit, parts_ammo, ammunition_ammo, parts_max, ammunition_max, parts_rate, ammunition_rate);
/// @param  target_unit
/// @param  parts_ammo
...
function scr_SupplyShip_Depot_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6) {
	...
	*/
	...
	var depotammo_supply_rate  = argument5;
	var depotparts_supply_rate = argument6;
	...
	var ap_cost = 2;
	    ...
	    if f1 {
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if global.depotParts_s_amount <= (global.lc_parts_ammo-depotparts_supply_rate) { 
	                //Make sure the unit rpg ammo does not go over max
	                if ((global.depotParts_s_amount+depotparts_count)+depotparts_supply_rate) <= parts_max {
	                    //Check if there is enough turn AP for the move
	                    var m_ap = (global.turn_AP-global.temp_AP);
	                    if m_ap >= ap_cost {
	                        //Check total unit AP available
	                        var ap = target_unit.action_points;
	                        if ap >= ((global.depotParts_s_amount+depotparts_supply_rate)/depotparts_supply_rate)*ap_cost {
	                            //Add the rounds
	                            global.depotParts_s_amount += depotparts_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if global.depotParts_s_amount >= depotparts_supply_rate { 
	                global.depotParts_s_amount -= depotparts_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if global.depotParts_s_amount < 0 { global.depotParts_s_amount = 0; }
	        if global.depotParts_s_amount > global.lc_parts_ammo { global.depotParts_s_amount = global.lc_parts_ammo; }
	        //Add to temp AP cost
	        global.temp_AP = (global.depotParts_s_amount/depotparts_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough supply available
	                if global.depotAmmo_s_amount <= (global.lc_ammunition_ammo-depotammo_supply_rate) {
	                    //Make sure the unit ammo does not go over max
	                    if ((global.depotAmmo_s_amount+depotammo_count)+depotammo_supply_rate) <= ammunition_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            var ap = target_unit.action_points;
	                            if ap >= ((global.depotAmmo_s_amount+depotammo_supply_rate)/depotammo_supply_rate)*ap_cost {
	                                //Add the rounds
	                                global.depotAmmo_s_amount +=  depotammo_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            //Decrease by a rate of 10
	            if mouse_wheel_down() { 
	                if global.depotAmmo_s_amount >= depotammo_supply_rate { 
	                    global.depotAmmo_s_amount -= depotammo_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if global.depotAmmo_s_amount < 0 { global.depotAmmo_s_amount = 0; }
	            if global.depotAmmo_s_amount > global.lc_ammunition_ammo { global.depotAmmo_s_amount = global.lc_ammunition_ammo; }
	            //Add to temp AP cost
	            global.temp_AP = (global.depotAmmo_s_amount/depotammo_supply_rate)*ap_cost;
	            //Set the menu position
	...
	if mouse_check_button_released(mb_left) {
	    if global.my_turn == true {
	        if f0 { 
	                ...
	                //Remove cost
	                if global.depotParts_s_amount != 0 {
	                    if global.supply_ship != noone {
	                        var u = global.supply_ship;
	                        if (u.can_shoot == true) && (u.parts_ammo >= global.depotParts_s_amount) {
	                            if (target_unit.action_points >= global.temp_AP) {
	                                ...
	                                u.shoot_amount    += global.depotParts_s_amount/depotparts_supply_rate; 
	                                u.parts_ammo      -= global.depotParts_s_amount;
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove ammo cost
	                    if global.depotAmmo_s_amount != 0 {
	                        if global.supply_ship != noone {
	                            var u = global.supply_ship;
	                            if (u.can_shoot == true) && (u.ammunition_ammo >= global.depotAmmo_s_amount) {
	                                if (target_unit.action_points >= global.temp_AP) {
	                                    u.action_confirmed = true;
	                                    global.units_running += 1; 
	                                    ...
	                                    u.resupply_unit = argument0;
	                                    u.resupply_unit.resupplying = true;
	                                    u.shoot_amount    += global.depotAmmo_s_amount/depotammo_supply_rate; 
	                                    u.ammunition_ammo -= global.depotAmmo_s_amount;
	                                    global.turn_AP    -= ap_cost*u.shoot_amount;
	                                    target_unit.alert_display = true;
	                                    target_unit.alert_colour = c_red;
										target_unit.alarm[3] = global.tick_rate*3;
