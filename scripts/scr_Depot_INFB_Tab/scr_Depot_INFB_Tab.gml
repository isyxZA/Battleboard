/// @description scr_Depot_INFB_Tab(target_unit, rifle_ammo, rpg_ammo, flare_ammo, rifle_max, rpg_max, flare_max, rifle_rate, rpg_rate, flare_rate);
/// @param target_unit
/// @param rifle_ammo
/// @param rpg_ammo
/// @param flare_ammo
...
/// @param rpg_rate
/// @param flare_rate
function scr_Depot_INFB_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6, argument7, argument8, argument9) {
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if infrfl_d_amount <= (dpt_amo_supply-rfl_supply_rate) {
	                //Make sure the unit rifle ammo does not go over max
	                if ((infrfl_d_amount+rifle_count)+rfl_supply_rate) <= rifle_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((infrfl_d_amount+rfl_supply_rate)/rfl_supply_rate)*ap_cost {
	                            //Add the rounds
	                            infrfl_d_amount += rfl_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if infrfl_d_amount >= rfl_supply_rate { 
	                infrfl_d_amount -= rfl_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if infrfl_d_amount < 0 { infrfl_d_amount = 0; }
	        if infrfl_d_amount > dpt_amo_supply { infrfl_d_amount = dpt_amo_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (infrfl_d_amount/rfl_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                ...
	                if infrpg_d_amount <= (dpt_amo_supply-rpg_supply_rate) { 
	                    //Make sure the unit rpg ammo does not go over max
	                    if ((infrpg_d_amount+rpg_count)+rpg_supply_rate) <= rpg_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((infrpg_d_amount+rpg_supply_rate)/rpg_supply_rate)*ap_cost {
	                                //Add the rounds
	                                infrpg_d_amount +=  rpg_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            if mouse_wheel_down() { 
	                if infrpg_d_amount >= rpg_supply_rate { 
	                    infrpg_d_amount -= rpg_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if infrpg_d_amount < 0 { infrpg_d_amount = 0; }
	            if infrpg_d_amount > dpt_amo_supply { infrpg_d_amount = dpt_amo_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (infrpg_d_amount/rpg_supply_rate)*ap_cost;
	            //Set the menu position
	            ...
	            obj_ACTIONMENU.display_menuinfo = true;
	                ...
	                if mouse_wheel_up() { 
	                    //Check if there is enough ammo available
	                    if infflr_d_amount <= (dpt_amo_supply-flr_supply_rate) { 
	                        //Make sure the unit flare ammo does not go over max
	                        if ((infflr_d_amount+flare_count)+flr_supply_rate) <= flare_max {
	                            //Check if there is enough turn AP for the move
	                            var m_ap = (global.turn_AP-global.temp_AP);
	                            if m_ap >=  ap_cost {
	                                //Check total unit AP available
	                                ...
	                                if ap >= ((infflr_d_amount+flr_supply_rate)/flr_supply_rate)*ap_cost {
	                                    //Add the rounds
	                                    infflr_d_amount +=  flr_supply_rate; 
	                                }
	                            }
	                        }
	                    } 
	                ...
	                //Decrease by a rate of 1
	                if mouse_wheel_down() { 
	                    if infflr_d_amount >= flr_supply_rate { 
	                        infflr_d_amount -= flr_supply_rate; 
	                    } 
	                }
	                //Clamp the value between zero and the max available rounds
	                if infflr_d_amount < 0 { infflr_d_amount = 0; }
	                if infflr_d_amount > dpt_amo_supply { infflr_d_amount = dpt_amo_supply; }
	                //Add to temp AP cost
	                global.temp_AP = (infflr_d_amount/flr_supply_rate)*ap_cost;
	                //Set the menu position
	    ...
	    if global.my_turn == true {
	        if f0 { 
				...
				if ds_list_size(tabs) > 1 {
	                ...
	                //TRANSFER RIFLE AMMO
	                //Remove ammo cost
	                if infrfl_d_amount != 0 {
	                    var t=0;
	                    while (infrfl_d_amount >= rfl_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                ...
	                queue_reset = true;
	                    ...
	                    //TRANSFER RPG AMMO
	                    //Remove ammo cost
	                    if infrpg_d_amount != 0 {
	                        var t=0;
	                        while (infrpg_d_amount >= rpg_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_depot_list, i);
	                                if (u.can_shoot == true) && (u.ammunition_ammo >= rpg_supply_rate) {
	                                    if (u.action_points >= ap_cost) {
	                                        ...
	                                        if infrpg_d_amount >= 0 { 
	                                            ...
	                                            u.shoot_amount    += 1; 
	                                            u.ammunition_ammo -= rpg_supply_rate;
	                                            global.turn_AP    -= ap_cost;
	                                            u.action_points   -= ap_cost;
	                                            u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                        }
	                    ...
	                    queue_reset = true;
	                        ...
	                        if infflr_d_amount != 0 {
	                            var t=0;
	                            while (infflr_d_amount >= flr_supply_rate) {
	                                t+=1;
	                                var i;
	                                for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                                    var u = ds_list_find_value(global.selected_depot_list, i);
