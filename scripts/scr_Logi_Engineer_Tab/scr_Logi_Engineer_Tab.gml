/// @description scr_Logi_Engineer_Tab(target_unit, depot_supply, repair_supply, tow_supply, mortar_supply, depot_max, repair_max, tow_max, mortar_max, depot_rate, repair_rate, tow_rate, mortar_rate);
/// @param target_unit
/// @param  depot_supply
/// @param  repair_supply
/// @param  tow_supply
...
/// @param  tow_rate
/// @param  mortar_rate
function scr_Logi_Engineer_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6, argument7, argument8, argument9, argument10, argument11, argument12) {
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if global.depot_l_amount <= (global.building_supply-depot_supply_rate) {
	                //Make sure the unit does not go over max
	                if ((global.depot_l_amount+depot_count)+depot_supply_rate) <= depot_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((global.depot_l_amount+depot_supply_rate)/depot_supply_rate)*ap_cost {
	                            //Add the rounds
	                            global.depot_l_amount +=  depot_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if global.depot_l_amount >= depot_supply_rate { 
	                global.depot_l_amount -= depot_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if global.depot_l_amount < 0 { global.depot_l_amount = 0; }
	        if global.depot_l_amount > global.building_supply { global.depot_l_amount = global.building_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (global.depot_l_amount/depot_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough building supply available
	                if global.repair_l_amount <= (global.building_supply-repair_supply_rate) {
	                    //Make sure the unit does not go over max
	                    if ((global.repair_l_amount+repair_count)+repair_supply_rate) <= repair_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((global.repair_l_amount+repair_supply_rate)/repair_supply_rate)*ap_cost {
	                                //Add the rounds
	                                global.repair_l_amount +=  repair_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            //Decrease by a rate of 1
	            if mouse_wheel_down() { 
	                if global.repair_l_amount >= repair_supply_rate { 
	                    global.repair_l_amount -= repair_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if global.repair_l_amount < 0 { global.repair_l_amount = 0; }
	            if global.repair_l_amount > global.building_supply { global.repair_l_amount = global.building_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (global.repair_l_amount/repair_supply_rate)*ap_cost;
	            //Set the menu position
	            ...
	            obj_ACTIONMENU.display_menuinfo = true;
	                ...
	                if mouse_wheel_up() { 
	                     //Check if there is enough building supply available
	                    if global.tow_l_amount <= (global.building_supply-tow_supply_rate) {
	                        //Make sure the unit rifle building does not go over max
	                        if ((global.tow_l_amount+tow_count)+tow_supply_rate) <= tow_max {
	                            //Check if there is enough turn AP for the move
	                            var m_ap = (global.turn_AP-global.temp_AP);
	                            if m_ap >=  ap_cost {
	                                //Check total unit AP available
	                                ...
	                                if ap >= ((global.tow_l_amount+tow_supply_rate)/tow_supply_rate)*ap_cost {
	                                    //Add the rounds
	                                    global.tow_l_amount +=  tow_supply_rate; 
	                                }
	                            }
	                        }
	                    } 
	                ...
	                //Decrease by a rate of 1
	                if mouse_wheel_down() { 
	                    if global.tow_l_amount >= tow_supply_rate { 
	                        global.tow_l_amount -= tow_supply_rate; 
	                    } 
	                }
	                //Clamp the value between zero and the max available rounds
	                if global.tow_l_amount < 0 { global.tow_l_amount = 0; }
	                if global.tow_l_amount > global.building_supply { global.tow_l_amount = global.building_supply; }
	                //Add to temp AP cost
	                global.temp_AP = (global.tow_l_amount/tow_supply_rate)*ap_cost;
	                ...
	                obj_ACTIONMENU.display_menuinfo = true;
	                    ...
	                    if mouse_wheel_up() { 
	                         //Check if there is enough building supply available
	                        if global.mortar_l_amount <= (global.building_supply-mortar_supply_rate) {
	                            //Make sure the unit does not go over max
	                            if ((global.mortar_l_amount+mortar_count)+mortar_supply_rate) <= mortar_max {
	                                //Check if there is enough turn AP for the move
	                                var m_ap = (global.turn_AP-global.temp_AP);
	                                if m_ap >=  ap_cost {
	                                    //Check total unit AP available
	                                    ...
	                                    if ap >= ((global.mortar_l_amount+mortar_supply_rate)/mortar_supply_rate)*ap_cost {
	                                        //Add the rounds
	                                        global.mortar_l_amount +=  mortar_supply_rate; 
	                                    }
	                                }
	                            }
	                        } 
	                    ...
	                    //Decrease by a rate of 1
	                    if mouse_wheel_down() { 
	                        if global.mortar_l_amount >= mortar_supply_rate { 
	                            global.mortar_l_amount -= mortar_supply_rate; 
	                        } 
	                    }
	                    //Clamp the value between zero and the max available rounds
	                    if global.mortar_l_amount < 0 { global.mortar_l_amount = 0; }
	                    if global.mortar_l_amount > global.building_supply { global.mortar_l_amount = global.building_supply; }
	                    //Add to temp AP cost
	                    global.temp_AP = (global.mortar_l_amount/mortar_supply_rate)*ap_cost;
	    ...
	    if global.my_turn == true {
	        if f0 { 
	            ...
	            if global.selected_depot != 0 {
	                ...
	                //Remove supply cost
	                if global.depot_l_amount != 0 {
	                    var t=0;
	                    while (global.depot_l_amount >= depot_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                            var u = ds_list_find_value(global.selected_logi_list, i);
	                            if (u.can_shoot == true) && (u.building_supply >= depot_supply_rate) {
	                                if (u.action_points >= ap_cost) {
	                                    ...
	                                    if global.depot_l_amount >= 0 { 
	                                        ...
	                                        u.shoot_amount     += 1; 
	                                        u.building_supply  -= depot_supply_rate;
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove supply cost
	                    if global.repair_l_amount != 0 {
	                        var t=0;
	                        while (global.repair_l_amount >= repair_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_logi_list, i);
	                                if (u.can_shoot == true) && (u.building_supply >= repair_supply_rate) {
	                                    if (u.action_points >= ap_cost) {
	                                        global.repair_l_amount -= repair_supply_rate;
	                                        if global.repair_l_amount >= 0 { 
	                                            if u.shoot_amount == 0 { 
	                                            ...
	                                            u.shoot_amount    += 1; 
	                                            u.building_supply -= repair_supply_rate;
	                                            global.turn_AP    -= ap_cost;
	                                            u.action_points   -= ap_cost;
	                                            u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                        }
	                    ...
	                    queue_reset = true;
	                        ...
	                        //TRANSFER TOW SUPPLY
	                        //Remove supply cost
	                        if global.tow_l_amount != 0 {
	                            var t=0;
	                            while (global.tow_l_amount >= tow_supply_rate) {
	                                t+=1;
	                                var i;
	                                for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                                    var u = ds_list_find_value(global.selected_logi_list, i);
	                                    if (u.can_shoot == true) && (u.building_supply >= tow_supply_rate) {
	                                        if (u.action_points >= ap_cost) {
	                                            global.tow_l_amount -= tow_supply_rate;
	                                            if global.tow_l_amount >= 0 { 
	                                                if u.shoot_amount == 0 { 
	                                                ...
	                                                u.shoot_amount    += 1; 
	                                                u.building_supply -= tow_supply_rate;
	                                                global.turn_AP    -= ap_cost;
	                                                u.action_points   -= ap_cost;
	                                                u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                            }
	                        ...
	                        queue_reset = true;
	                            ...
	                            //Remove supply cost
	                            if global.mortar_l_amount != 0 {
	                                var t=0;
	                                while (global.mortar_l_amount >= mortar_supply_rate) {
	                                    t+=1;
	                                    var i;
	                                    for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                                        var u = ds_list_find_value(global.selected_logi_list, i);
	                                        if (u.can_shoot == true) && (u.building_supply >= mortar_supply_rate) {
	                                            if (u.action_points >= ap_cost) {
	                                                global.mortar_l_amount -= mortar_supply_rate;
	                                                if global.flare_l_amount >= 0 { 
	                                                    if u.shoot_amount == 0 { 
	                                                        u.action_confirmed = true;
	                                                        global.units_running += 1; 
	                                                        u.shoot_building = true;
	                                                    ...
	                                                    u.shoot_amount    += 1; 
	                                                    u.building_supply -= mortar_supply_rate;
	                                                    global.turn_AP    -= ap_cost;
	                                                    u.action_points   -= ap_cost;
	                                                    u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                                }
