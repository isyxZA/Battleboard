/// @description scr_Logi_Logi_Tab(target_unit, ammo_supply, building_supply, ammo_max, building_max, ammo_rate, building_rate);
/// @param target_unit
/// @param  ammo_supply
...
/// @param  ammo_rate
/// @param  building_rate
function scr_Logi_Logi_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6) {
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if global.ammo_l_amount <= (global.ammo_supply-ammo_supply_rate) {
	                //Make sure the unit ammo does not go over max
	                if ((global.ammo_l_amount+ammo_count)+ammo_supply_rate) <= ammo_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((global.ammo_l_amount+ammo_supply_rate)/ammo_supply_rate)*ap_cost {
	                            //Add the rounds
	                            global.ammo_l_amount +=  ammo_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if global.ammo_l_amount >= ammo_supply_rate { 
	                global.ammo_l_amount -= ammo_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if global.ammo_l_amount < 0 { global.ammo_l_amount = 0; }
	        if global.ammo_l_amount > global.ammo_supply { global.ammo_l_amount = global.ammo_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (global.ammo_l_amount/ammo_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough supply available
	                if global.building_l_amount <= (global.building_supply-building_supply_rate) { 
	                    //Make sure the unit rpg ammo does not go over max
	                    if ((global.building_l_amount+building_count)+building_supply_rate) <= building_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((global.building_l_amount+building_supply_rate)/building_supply_rate)*ap_cost {
	                                //Add the rounds
	                                global.building_l_amount +=  building_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            if mouse_wheel_down() { 
	                if global.building_l_amount >= building_supply_rate { 
	                    global.building_l_amount -= building_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if global.building_l_amount < 0 { global.building_l_amount = 0; }
	            if global.building_l_amount > global.building_supply { global.building_l_amount = global.building_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (global.building_l_amount/building_supply_rate)*ap_cost;
	            //Set the menu position
	    ...
	    if global.my_turn == true {
	        if f0 { 
	            ...
	            if global.selected_depot != 0 {
	                ...
	                //Remove ammo cost
	                if global.ammo_l_amount != 0 {
	                    var t=0;
	                    while (global.ammo_l_amount >= ammo_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                            var u = ds_list_find_value(global.selected_logi_list, i);
	                            if (u.can_shoot == true) && (u.ammo_supply >= ammo_supply_rate) {
	                                if (u.action_points >= ap_cost) {
	                                    ...
	                                    if global.ammo_l_amount >= 0 { 
	                                        ...
	                                        u.shoot_amount   += 1; 
	                                        u.ammo_supply    -= ammo_supply_rate;
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove ammo cost
	                    if global.building_l_amount != 0 {
	                        var t=0;
	                        while (global.building_l_amount >= building_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_logi_list, i);
	                                if (u.can_shoot == true) && (u.building_supply >= building_supply_rate) {
	                                    if (u.action_points >= ap_cost) {
	                                        global.building_l_amount -= building_supply_rate;
	                                        if global.building_l_amount >= 0 { 
	                                            if u.shoot_amount == 0 { 
	                                            ...
	                                            u.shoot_amount    += 1; 
	                                            u.building_supply -= building_supply_rate;
	                                            global.turn_AP    -= ap_cost;
	                                            u.action_points   -= ap_cost;
	                                            u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                        }
