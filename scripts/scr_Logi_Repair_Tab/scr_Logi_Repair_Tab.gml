/// @description scr_Logi_Repair_Tab(target_unit, repair_ammo, mg_ammo, repair_max, mg_max, repair_rate, mg_rate);
/// @param target_unit
/// @param  repair_ammo
/// @param  mg_ammo
/// @param  repair_max
...
/// @param  repair_rate
/// @param  mg_rate
function scr_Logi_Repair_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6) {

	/*  Arguments
	0   resupply unit ID        
	1   repair count     
	2   mg ammo count                
	3   max repair the unit can carry
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if global.repairS_l_amount <= (global.building_supply-repairS_supply_rate) { 
	                //Make sure the unit rpg ammo does not go over max
	                if ((global.repairS_l_amount+repair_count)+repairS_supply_rate) <= repair_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((global.repairS_l_amount+repairS_supply_rate)/repairS_supply_rate)*ap_cost {
	                            //Add the rounds
	                            global.repairS_l_amount +=  repairS_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if global.repairS_l_amount >= repairS_supply_rate { 
	                global.repairS_l_amount -= repairS_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if global.repairS_l_amount < 0 { global.repairS_l_amount = 0; }
	        if global.repairS_l_amount > global.building_supply { global.repairS_l_amount = global.building_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (global.repairS_l_amount/repairS_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough supply available
	                if global.repairmg_l_amount <= (global.ammo_supply-repairmg_supply_rate) {
	                    //Make sure the unit ammo does not go over max
	                    if ((global.repairmg_l_amount+repairmg_count)+repairmg_supply_rate) <= mg_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((global.repairmg_l_amount+repairmg_supply_rate)/repairmg_supply_rate)*ap_cost {
	                                //Add the rounds
	                                global.repairmg_l_amount += repairmg_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            if mouse_wheel_down() { 
	                if global.repairmg_l_amount >= repairmg_supply_rate { 
	                    global.repairmg_l_amount -= repairmg_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if global.repairmg_l_amount < 0 { global.repairmg_l_amount = 0; }
	            if global.repairmg_l_amount > global.ammo_supply { global.repairmg_l_amount = global.ammo_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (global.repairmg_l_amount/repairmg_supply_rate)*ap_cost;
	            //Set the menu position
	    ...
	    if global.my_turn == true {
	        if f0 { 
	            ...
	            if global.selected_depot != 0 {
	                ...
	                //Remove cost
	                if global.repairS_l_amount != 0 {
	                    var t=0;
	                    while (global.repairS_l_amount >= repairS_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove ammo cost
	                    if global.repairmg_l_amount != 0 {
	                        var t=0;
	                        while (global.repairmg_l_amount >= repairmg_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_logi_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_logi_list, i);
