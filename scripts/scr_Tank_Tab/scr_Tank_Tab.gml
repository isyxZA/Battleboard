/// @description scr_Tank_Tab(cannon_rate, mg_rate, cannon_cost, mg_cost)
/// @param cannon_rate
/// @param  mg_rate
/// @param  cannon_cost
/// @param  mg_cost
function scr_Tank_Tab(argument0, argument1, argument2, argument3) {
	...
	if f0 { 
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if global.cannon_amount <= (global.cannon_ammo-cannon_rate) { 
	                ...
	                if m_ap >= cannon_cost {
	                    ...
	                    var i;
	                    var ap = 0;
	                    if !ds_list_empty(global.selected_tank_list) {
	                        for (i=0; i<ds_list_size(global.selected_tank_list); i+=1) {
	                            var unit = ds_list_find_value(global.selected_tank_list, i);
	                            if unit.can_shoot == true { ap += unit.action_points; }
	                        }
	                    }
	                    if ap >= ((global.cannon_amount+cannon_rate)/cannon_rate)*cannon_cost {
	                            //Add the rounds
	                            global.cannon_amount += cannon_rate; 
	                    }
	                }
	            } 
	        }
	        ...
	        if mouse_wheel_down() { 
	            if global.cannon_amount >= cannon_rate { 
	                global.cannon_amount -= cannon_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if global.cannon_amount < 0 { global.cannon_amount = 0; }
	        if global.cannon_amount > global.cannon_ammo { global.cannon_amount = global.cannon_ammo; }
	        //Add to temp AP cost
	        global.temp_AP = (global.cannon_amount/cannon_rate)*cannon_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough ammo available
	                if global.mg_tank_amount <= (global.mg_tank_ammo-mg_rate) { 
	                    //Check if there is enough turn AP for the move
	                    var m_ap = (global.turn_AP-global.temp_AP);
	                    if m_ap >=  mg_cost {
	                        //Check total unit AP available
	                        ...
	                        if ap >= ((global.mg_tank_amount+mg_rate)/mg_rate)*mg_cost {
	                            //Add the rounds
	                            global.mg_tank_amount += mg_rate; 
	                        }
	                    }
	                } 
	            }
	            ...
	            if mouse_wheel_down() { 
	                if global.mg_tank_amount >= mg_rate { 
	                    global.mg_tank_amount -= mg_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if global.mg_tank_amount < 0 { global.mg_tank_amount = 0; }
	            if global.mg_tank_amount > global.mg_tank_ammo { global.mg_tank_amount = global.mg_tank_ammo; }
	            //Add to temp AP cost
	            global.temp_AP = (global.mg_tank_amount/mg_rate)*mg_cost;
	    ...
	    if global.my_turn == true {
	        if f0 { 
	            ...
	            if global.selected_engineer != 0 {
	                ...
	                //FIRE MAIN CANNON
	                //Remove ammo cost
	                if global.cannon_amount != 0 {
	                    var t=0;
	                    while (global.cannon_amount >= cannon_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_tank_list); i+=1) {
	                            var u = ds_list_find_value(global.selected_tank_list, i);
	                ...
	                queue_reset = true;
	                    ...
	                    //FIRE MACHINE GUN
	                    //Remove ammo cost
	                    if global.mg_tank_amount != 0 {
	                        var t=0;
	                        while (global.mg_tank_amount >= mg_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_tank_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_tank_list, i);
