/// @description scr_LOGIA_LOGIA_Tab(target_unit, ammo_supply, building_supply, ammo_max, building_max, ammo_rate, building_rate);
/// @param target_unit
/// @param ammo_supply
...
/// @param ammo_rate
/// @param building_rate
function scr_LOGIA_LOGIA_Tab(argument0, argument1, argument2, argument3, argument4, argument5, argument6) {
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if global.logiamo_l_amount <= (global.logi_amo_supply-logiamo_supply_rate) {
	        ...
	        if global.logiamo_l_amount > global.logi_amo_supply { global.logiamo_l_amount = global.logi_amo_supply; }
	        //Add to temp AP cost
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough supply available
	                if global.logibld_l_amount <= (global.logi_bld_supply-logibld_supply_rate) { 
	                    //Make sure the unit rpg ammo does not go over max
	                    if ((global.logibld_l_amount+building_count)+logibld_supply_rate) <= building_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((global.logibld_l_amount+logibld_supply_rate)/logibld_supply_rate)*ap_cost {
	                                //Add the rounds
	                                global.logibld_l_amount +=  logibld_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            if mouse_wheel_down() { 
	                if global.logibld_l_amount >= logibld_supply_rate { 
	                    global.logibld_l_amount -= logibld_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if global.logibld_l_amount < 0 { global.logibld_l_amount = 0; }
	            if global.logibld_l_amount > global.logi_bld_supply { global.logibld_l_amount = global.logi_bld_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (global.logibld_l_amount/logibld_supply_rate)*ap_cost;
	            //Set the menu position
	        ...
	        if f0 { 
				...
				if global.selected_logib != 0 {
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove ammo cost
	                    if global.logibld_l_amount != 0 {
	                        var t=0;
	                        while (global.logibld_l_amount >= logibld_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_logia_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_logia_list, i);
	                                if (u.can_shoot == true) && (u.building_supply >= logibld_supply_rate) {
	                                    if (u.action_points >= ap_cost) {
	                                        global.logibld_l_amount -= logibld_supply_rate;
	                                        if global.logibld_l_amount >= 0 { 
	                                            if u.shoot_amount == 0 { 
	                                            ...
	                                            u.shoot_amount    += 1; 
	                                            u.building_supply -= logibld_supply_rate;
	                                            global.turn_AP    -= ap_cost;
	                                            u.action_points   -= ap_cost;
	                                            u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                        }
