	...
	*/
	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	            ...
	            if global.dptbld_d_amount <= (global.dpt_bld_supply-depotbld_supply_rate) { 
	                ...
	                if ((global.dptbld_d_amount+depotparts_count)+depotbld_supply_rate) <= parts_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((global.dptbld_d_amount+depotbld_supply_rate)/depotbld_supply_rate)*ap_cost {
	                            //Add the rounds
	                            global.dptbld_d_amount +=  depotbld_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if global.dptbld_d_amount >= depotbld_supply_rate { 
	                global.dptbld_d_amount -= depotbld_supply_rate; 
	            } 
	        }
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	                ...
	                if global.dptamo_d_amount <= (global.dpt_amo_supply-depotamo_supply_rate) {
	                    ...
	                    if ((global.dptamo_d_amount+depotammo_count)+depotamo_supply_rate) <= ammunition_max {
	                        ...
	                        if m_ap >=  ap_cost {
	                            ...
	                            if ap >= ((global.dptamo_d_amount+depotamo_supply_rate)/depotamo_supply_rate)*ap_cost {
	                                //Add the rounds
	                                global.dptamo_d_amount +=  depotamo_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            if mouse_wheel_down() { 
	                if global.dptamo_d_amount >= depotamo_supply_rate { 
	                    global.dptamo_d_amount -= depotamo_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	    ...
	    if global.my_turn == true {
	        if f0 { 
				...
				if global.selected_repair != 0 {
	                ...
	                //Remove cost
	                if global.dptbld_d_amount != 0 {
	                    var t=0;
	                    while (global.dptbld_d_amount >= depotbld_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove ammo cost
	                    if global.dptamo_d_amount != 0 {
	                        var t=0;
	                        while (global.dptamo_d_amount >= depotamo_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_depot_list, i);
