	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if dptbld_d_amount <= (dpt_bld_supply-depotbld_supply_rate) { 
	                //Make sure the unit rpg ammo does not go over max
	                if ((dptbld_d_amount+depotparts_count)+depotbld_supply_rate) <= parts_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((dptbld_d_amount+depotbld_supply_rate)/depotbld_supply_rate)*ap_cost {
	                            //Add the rounds
	                            dptbld_d_amount +=  depotbld_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if dptbld_d_amount >= depotbld_supply_rate { 
	                dptbld_d_amount -= depotbld_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if dptbld_d_amount < 0 { dptbld_d_amount = 0; }
	        if dptbld_d_amount > dpt_bld_supply { dptbld_d_amount = dpt_bld_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (dptbld_d_amount/depotbld_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough supply available
	                if dptamo_d_amount <= (dpt_amo_supply-depotamo_supply_rate) {
	                    //Make sure the unit ammo does not go over max
	                    if ((dptamo_d_amount+depotammo_count)+depotamo_supply_rate) <= ammunition_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((dptamo_d_amount+depotamo_supply_rate)/depotamo_supply_rate)*ap_cost {
	                                //Add the rounds
	                                dptamo_d_amount +=  depotamo_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            if mouse_wheel_down() { 
	                if dptamo_d_amount >= depotamo_supply_rate { 
	                    dptamo_d_amount -= depotamo_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if dptamo_d_amount < 0 { dptamo_d_amount = 0; }
	            if dptamo_d_amount > dpt_amo_supply { dptamo_d_amount = dpt_amo_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (dptamo_d_amount/depotamo_supply_rate)*ap_cost;
	            //Set the menu position
	    ...
	    if global.my_turn == true {
	        if f0 { 
				...
				if ds_list_size(tabs) > 1 {
	                ...
	                //Remove cost
	                if dptbld_d_amount != 0 {
	                    var t=0;
	                    while (dptbld_d_amount >= depotbld_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                            var u = ds_list_find_value(global.selected_depot_list, i);
	                            if (u.can_shoot == true) && (u.parts_ammo >= depotbld_supply_rate) {
	                                if (u.action_points >= ap_cost) {
	                                    ...
	                                    if dptbld_d_amount >= 0 { 
	                                        ...
	                                        u.shoot_amount    += 1; 
	                                        u.parts_ammo      -= depotbld_supply_rate;
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove ammo cost
	                    if dptamo_d_amount != 0 {
	                        var t=0;
	                        while (dptamo_d_amount >= depotamo_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_depot_list, i);
	                                if (u.can_shoot == true) && (u.ammunition_ammo >= depotamo_supply_rate) {
	                                    if (u.action_points >= ap_cost) {
	                                        dptamo_d_amount -= depotamo_supply_rate;
	                                        if dptamo_d_amount >= 0 { 
	                                            if u.shoot_amount == 0 { 
	                                            ...
	                                            u.shoot_amount    += 1; 
	                                            u.ammunition_ammo -= depotamo_supply_rate;
	                                            global.turn_AP    -= ap_cost;
	                                            u.action_points   -= ap_cost;
	                                            u.alert_text = "-"+string(u.shoot_amount*ap_cost)+ " AP";
	                                        }
