	...
	if surface_exists(global.menu_surf) {
		...
		for (m=0; m<mlen; m+=1){
		    ...
		    if (m < mlen-1) {
				...
				draw_text_colour_shadow(0+(global.menu_width*0.5), (0+(global.menu_height*0.5))+(m*global.menu_height), r_vehicle_menu[m] + " " + "x" + " " + string(shoot_amount), c1, c1, c1, c1, 1, 300, c_black, 0.2);
		    }
		        else {
		            var c3;
		            if m == global.fire_option { c3 = c_red; } else { c3 = c_gray; }
		            //For CANCEL
		            draw_text_colour_shadow(0+(global.menu_width*0.5), (0+(global.menu_height*0.5))+(m*global.menu_height), r_vehicle_menu[m] + " " + "x" + " " + string(shoot_amount), c3, c3, c3, c3, 1, 300, c_black, 0.2);
		        }
