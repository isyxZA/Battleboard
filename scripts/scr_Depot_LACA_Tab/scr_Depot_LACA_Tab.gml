	...
	if f0 {
	    ...
	    obj_ACTIONMENU.display_menuinfo = false;
	        ...
	        if mouse_wheel_up() { 
	            ...
	            if lacdpt_d_amount <= (dpt_bld_supply-lacdpt_supply_rate) {
	                //Make sure the unit does not go over max
	                if ((lacdpt_d_amount+depot_count)+lacdpt_supply_rate) <= depot_max {
	                    ...
	                    if m_ap >=  ap_cost {
	                        ...
	                        if ap >= ((lacdpt_d_amount+lacdpt_supply_rate)/lacdpt_supply_rate)*ap_cost {
	                            //Add the rounds
	                            lacdpt_d_amount +=  lacdpt_supply_rate; 
	                        }
	                    }
	                }
	            } 
	        ...
	        if mouse_wheel_down() { 
	            if lacdpt_d_amount >= lacdpt_supply_rate { 
	                lacdpt_d_amount -= lacdpt_supply_rate; 
	            } 
	        }
	        //Clamp the value between zero and the max available rounds
	        if lacdpt_d_amount < 0 { lacdpt_d_amount = 0; }
	        if lacdpt_d_amount > dpt_bld_supply { lacdpt_d_amount = dpt_bld_supply; }
	        //Add to temp AP cost
	        global.temp_AP = (lacdpt_d_amount/lacdpt_supply_rate)*ap_cost;
	        ...
	        obj_ACTIONMENU.display_menuinfo = true;
	            ...
	            if mouse_wheel_up() { 
	                //Check if there is enough building supply available
	                if lacrpr_d_amount <= (dpt_bld_supply-lacrpr_supply_rate) {
	                    //Make sure the unit does not go over max
	                    if ((lacrpr_d_amount+repair_count)+lacrpr_supply_rate) <= repair_max {
	                        //Check if there is enough turn AP for the move
	                        var m_ap = (global.turn_AP-global.temp_AP);
	                        if m_ap >=  ap_cost {
	                            //Check total unit AP available
	                            ...
	                            if ap >= ((lacrpr_d_amount+lacrpr_supply_rate)/lacrpr_supply_rate)*ap_cost {
	                                //Add the rounds
	                                lacrpr_d_amount +=  lacrpr_supply_rate; 
	                            }
	                        }
	                    }
	                } 
	            ...
	            //Decrease by a rate of 1
	            if mouse_wheel_down() { 
	                if lacrpr_d_amount >= lacrpr_supply_rate { 
	                    lacrpr_d_amount -= lacrpr_supply_rate; 
	                } 
	            }
	            //Clamp the value between zero and the max available rounds
	            if lacrpr_d_amount < 0 { lacrpr_d_amount = 0; }
	            if lacrpr_d_amount > dpt_bld_supply { lacrpr_d_amount = dpt_bld_supply; }
	            //Add to temp AP cost
	            global.temp_AP = (lacrpr_d_amount/lacrpr_supply_rate)*ap_cost;
	            //Set the menu position
	            ...
	            obj_ACTIONMENU.display_menuinfo = true;
	                ...
	                if mouse_wheel_up() { 
	                     //Check if there is enough building supply available
	                    if lactow_d_amount <= (dpt_bld_supply-lactow_supply_rate) {
	                        //Make sure the unit rifle building does not go over max
	                        if ((lactow_d_amount+tow_count)+lactow_supply_rate) <= tow_max {
	                            //Check if there is enough turn AP for the move
	                            var m_ap = (global.turn_AP-global.temp_AP);
	                            if m_ap >=  ap_cost {
	                                //Check total unit AP available
	                                ...
	                                if ap >= ((lactow_d_amount+lactow_supply_rate)/lactow_supply_rate)*ap_cost {
	                                    //Add the rounds
	                                    lactow_d_amount +=  lactow_supply_rate; 
	                                }
	                            }
	                        }
	                    } 
	                ...
	                //Decrease by a rate of 1
	                if mouse_wheel_down() { 
	                    if lactow_d_amount >= lactow_supply_rate { 
	                        lactow_d_amount -= lactow_supply_rate; 
	                    } 
	                }
	                //Clamp the value between zero and the max available rounds
	                if lactow_d_amount < 0 { lactow_d_amount = 0; }
	                if lactow_d_amount > dpt_bld_supply { lactow_d_amount = dpt_bld_supply; }
	                //Add to temp AP cost
	                global.temp_AP = (lactow_d_amount/lactow_supply_rate)*ap_cost;
	                ...
	                obj_ACTIONMENU.display_menuinfo = true;
	                    ...
	                    if mouse_wheel_up() { 
	                         //Check if there is enough building supply available
	                        if lacmtr_d_amount <= (dpt_bld_supply-lacmtr_supply_rate) {
	                            //Make sure the unit does not go over max
	                            if ((lacmtr_d_amount+mortar_count)+lacmtr_supply_rate) <= mortar_max {
	                                //Check if there is enough turn AP for the move
	                                var m_ap = (global.turn_AP-global.temp_AP);
	                                if m_ap >= ap_cost {
	                                    //Check total unit AP available
	                                    ...
	                                    if ap >= ((lacmtr_d_amount+lacmtr_supply_rate)/lacmtr_supply_rate)*ap_cost {
	                                        //Add the rounds
	                                        lacmtr_d_amount += lacmtr_supply_rate; 
	                                    }
	                                }
	                            }
	                        } 
	                    ...
	                    //Decrease by a rate of 1
	                    if mouse_wheel_down() { 
	                        if lacmtr_d_amount >= lacmtr_supply_rate { 
	                            lacmtr_d_amount -= lacmtr_supply_rate; 
	                        } 
	                    }
	                    //Clamp the value between zero and the max available rounds
	                    if lacmtr_d_amount < 0 { lacmtr_d_amount = 0; }
	                    if lacmtr_d_amount > dpt_bld_supply { lacmtr_d_amount = dpt_bld_supply; }
	                    //Add to temp AP cost
	                    global.temp_AP = (lacmtr_d_amount/lacmtr_supply_rate)*ap_cost;
	    ...
	    if global.my_turn == true {
	        if f0 { 
				...
				if ds_list_size(tabs) > 1 {
	                ...
	                //Remove supply cost
	                if lacdpt_d_amount != 0 {
	                    var t=0;
	                    while (lacdpt_d_amount >= lacdpt_supply_rate) {
	                        t+=1;
	                        var i;
	                        for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                ...
	                queue_reset = true;
	                    ...
	                    //Remove supply cost
	                    if lacrpr_d_amount != 0 {
	                        var t=0;
	                        while (lacrpr_d_amount >= lacrpr_supply_rate) {
	                            t+=1;
	                            var i;
	                            for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                                var u = ds_list_find_value(global.selected_depot_list, i);
	                    ...
	                    queue_reset = true;
	                        ...
	                        //TRANSFER TOW SUPPLY
	                        //Remove supply cost
	                        if lactow_d_amount != 0 {
	                            var t=0;
	                            while (lactow_d_amount >= lactow_supply_rate) {
	                                t+=1;
	                                var i;
	                                for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                                    var u = ds_list_find_value(global.selected_depot_list, i);
	                        ...
	                        queue_reset = true;
	                            ...
	                            //Remove supply cost
	                            if lacmtr_d_amount != 0 {
	                                var t=0;
	                                while (lacmtr_d_amount >= lacmtr_supply_rate) {
	                                    t+=1;
	                                    var i;
	                                    for (i=0; i<ds_list_size(global.selected_depot_list); i+=1) {
	                                        var u = ds_list_find_value(global.selected_depot_list, i);
