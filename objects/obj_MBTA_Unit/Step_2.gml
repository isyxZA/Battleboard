if nav_confirmed == true {
    ...
    if can_move == true {
        ...
        //Add alert to gui
        ds_list_add(global.action_alert_list, "Tank Moving");
    }
...
if action_confirmed == true {
    ...
    if shoot_cannon == true {
        if !shoot_mask.t_line {
            //Add alert to gui
            ds_list_add(global.action_alert_list, "Tank Firing Cannon"); 
            shoot_cannon = false;
            timer_target = 3;
            ...
            if !shoot_mask.t_line {
                //Add alert to gui
                ds_list_add(global.action_alert_list, "Tank Firing MG");
                shoot_mg = false;
                timer_target = 3;
                timer_count = timer_target;
                timer_start = true;
    ...
    if turret_rot != d2  { turret_rot += (sin(degtorad(turret_rot-d2))); }
        ...
        if selected && global.reticule_display == true {
             ...
             if turret_rot != d2  { turret_rot += (sin(degtorad(turret_rot-d2))); } 
				...
				if turret_rot != rot_offset  { turret_rot -= (sin(degtorad(turret_rot-rot_offset))); } 
			}
    }
...
if ammo_check == true {
    ...
    if (cannon_ammo < cannon_rate) && (mg_ammo < mg_rate) { 
        ...
        else if (cannon_ammo < cannon_rate) || (mg_ammo < mg_rate) {
            alert_display = true;
            alert_text = "Low Ammo!"; 
