...
if explode == true {
    if play_effect == true { 
        ...
        audio_play_sound_on(emit, snd_Explosion07, false, 10);
        var l = audio_sound_length(snd_Explosion07);
        alarm[0] = room_speed*l;
		with obj_BTR_PFX {
            if explode_ap1 == false {
                explode_ap1 = true;
                ap_x = other.x;
                ap_y = other.y;
            }
    ...
    if light_size < 2 { light_size = lerp(light_size, 2, 0.01); }
    if can_damage == true { 
        can_damage = false; 
